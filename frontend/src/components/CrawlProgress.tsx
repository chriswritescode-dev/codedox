...
export default function CrawlProgress({ job, currentUrl }: CrawlProgressProps) {
  const getPhaseBadge = () => {
    ...
    return (
      <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm ${config.color}`}>
        {config.icon}
        {config.label}
      </div>
    )
  ...
  const getTimeEstimate = () => {
    if (!job.started_at || job.urls_crawled === 0) return null
    
    const startTime = new Date(job.started_at).getTime()
    const currentTime = new Date().getTime()
    const elapsedMs = currentTime - startTime
    const avgTimePerPage = elapsedMs / job.urls_crawled
    const remainingPages = job.total_pages - job.urls_crawled
  ...
  return (
    <div className="bg-secondary/50 rounded-lg p-6 space-y-4">
      ...
      <div>
        <ProgressBar 
          current={job.crawl_progress || Math.min(100, Math.round((job.urls_crawled / job.total_pages * 100) || 0))} 
          total={100}
          label="Pages Crawled"
        />
        <div className="flex justify-between text-sm text-muted-foreground mt-1">
          <span>{job.urls_crawled} of {job.total_pages} pages</span>
          {job.status === 'running' && (
      ...
      {/* Enrichment Progress */}
      {job.crawl_phase === 'enriching' && job.documents_crawled && job.documents_enriched !== undefined && (
        <div>
          <ProgressBar 
            current={job.enrichment_progress || Math.min(100, Math.round((job.documents_enriched / job.documents_crawled * 100) || 0))} 
            total={100}
      ...
      <div className="grid grid-cols-2 gap-4 pt-2">
        ...
        <div>
          ...
          <p className="text-xl font-semibold">
            {job.urls_crawled > 0 ? (job.snippets_extracted / job.urls_crawled).toFixed(1) : '0'}
          </p>
        </div>
      </div>
    </div>
