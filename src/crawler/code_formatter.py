...
class CodeFormatter:
    ...
    def __init__(self):
        ...
        if not self.prettier_available:
            logger.warning(
                "Prettier not found. Run 'npm install' in the frontend directory to enable JavaScript/TypeScript formatting"
            )

    def _find_prettier_path(self) -> str | None:
    ...
    def format(self, code: str, language: str | None = None) -> str:
        """
        ...
        # Default: for unknown languages, return as-is
        else:
            return code
        ...
        try:
            ...
            with secure_temp_file(suffix=".json") as config_path:
                ...
                if result.returncode == 0:
                    ...
                    logger.warning(
                        f"Prettier formatting failed with parser '{parser}': {result.stderr}"
                    )
                    ...
                    for fallback_parser in fallback_parsers:
                        ...
                        if fallback_result:
                            logger.info(
                                f"Successfully formatted with fallback parser: {fallback_parser}"
                            )
    ...
    def _format_html(self, code: str) -> str:
        ...
        # Return as-is if Prettier not available
        return code
    ...
    def _format_with_auto_detection(self, code: str) -> str:
        ...
        if detected_lang:
            ...
            return self.format(code, detected_lang)

        # If we can't detect the language, return as-is
        return code

    def _try_prettier_with_extension(self, code: str, extension: str) -> str | None:
    ...
    def _try_prettier_auto_detect(self, code: str) -> str | None:
        ...
        try:
            with secure_temp_file(suffix=".json") as config_path:
                ...
                result = subprocess.run(
                    [self.prettier_path, "--config", config_path],
                    input=code,
                    capture_output=True,
                    text=True,
                    check=False,
    ...
    def _detect_language_from_patterns(self, code: str) -> str | None:
        ...
        # Python detection
        if any(
            pattern in code for pattern in ["def ", "class ", "import ", "from ", "if __name__"]
        ):
            if re.search(r"\bdef\s+\w+\s*\([^)]*\):", code) or re.search(
                r"\bclass\s+\w+\s*[\(:]", code
            ):
                return "python"
        ...
        if any(
            pattern in code
            for pattern in ["function ", "const ", "let ", "var ", "=>", "export ", "import "]
        ):
            # React JSX
        ...
        # CSS detection
        if re.search(r"[.#]?\w+\s*{[^}]*}", code) and any(
            prop in code
            for prop in ["color:", "background:", "margin:", "padding:", "display:", "position:"]
        ):
            # SCSS specific
    ...
    def format_with_info(self, code: str, language: str | None = None) -> dict[str, Any]:
        ...
        if formatted != code:
            if (
                ...
                formatter_used = "none"  # No formatting applied
