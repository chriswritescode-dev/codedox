        ...
        for file in files:
            try:
                ...
                if not (file.filename.endswith('.md') or 
                        file.filename.endswith('.markdown') or 
                ...
                if llm_generator and code_blocks:
                    try:
                        ...
                        enhanced_blocks = await llm_generator.generate_descriptions(
                            code_blocks,
                            file_url,
                            content_str  # Pass full content for context
                        )
                ...
                for block in code_blocks:
                    snippet = CodeSnippet(
                        ...
                        language=block.detected_language or block.language or 'text',
                        code_content=block.code_content,
                        code_hash=hashlib.sha256(block.code_content.encode()).hexdigest(),
