...
class TestHealthEndpoints:
    ...
    def test_llm_health(self, client, mock_llm_client):
...
class TestStatisticsEndpoint:
    ...
    def test_statistics_with_data(self, client, sample_crawl_job, sample_document, sample_code_snippets):
        ...
        assert recent["snippets_extracted"] == 25
...
class TestSourcesEndpoints:
    ...
    def test_get_sources_with_data(self, client, sample_crawl_job, sample_document, sample_code_snippets):
    ...
    def test_get_source_by_id(self, client, sample_crawl_job, sample_document, sample_code_snippets):
...
class TestCrawlJobEndpoints:
    ...
    def test_get_crawl_jobs_with_data(self, client, multiple_crawl_jobs):
        ...
        for job in jobs:
            ...
            assert isinstance(job["enrichment_progress"], int)
    ...
    def test_get_crawl_job_by_id(self, client, sample_crawl_job):
        ...
        # Enrichment progress depends on documents_enriched/documents_crawled
        assert 0 <= job["enrichment_progress"] <= 100
...
class TestBulkDeleteEndpoints:
    ...
    def test_delete_sources_bulk_nonexistent(self, client):
        ...
        assert response.status_code == 404
        assert response.json()["detail"] in ["No sources found with provided IDs", "Resource not found"]
    ...
    def test_delete_sources_bulk_with_documents(self, client, db, sample_crawl_job, sample_document, sample_code_snippets):
        ...
        assert db.query(CodeSnippet).filter(CodeSnippet.document_id == sample_document.id).count() > 0
    ...
    def test_delete_crawl_jobs_bulk_nonexistent(self, client):
        ...
        assert response.status_code == 404
        assert response.json()["detail"] in ["No deletable jobs found with provided IDs", "Resource not found"]
    ...
    def test_delete_crawl_jobs_bulk_success(self, client, db, multiple_crawl_jobs):
        ...
        non_deletable_jobs = [job for job in multiple_crawl_jobs if job.status not in deletable_statuses]
    ...
    def test_delete_crawl_jobs_bulk_with_cascade(self, client, db, sample_crawl_job, sample_document, sample_code_snippets):
        """Test crawl job bulk deletion cascades properly."""
        from src.database.models import CrawlJob, Document, CodeSnippet, PageLink
        ...
        assert db.query(PageLink).filter(PageLink.crawl_job_id == sample_crawl_job.id).count() == 2
        ...
        assert db.query(PageLink).filter(PageLink.crawl_job_id == sample_crawl_job.id).count() == 0
